...
function FillBlankQ({ question, onAnswer }: { question: GameEntry; onAnswer: AnswerFn }) {
  const q = question as any
...
function SortSentenceQ({ question, onAnswer }: { question: GameEntry; onAnswer: AnswerFn }) {
  const q = question as any
...
function WordScrambleQ({ question, onAnswer }: { question: GameEntry; onAnswer: AnswerFn }) {
  const q = question as any
  const letters: string[] = q.word.split('')
...
function CategorizeQ({ question, onAnswer }: { question: GameEntry; onAnswer: AnswerFn }) {
  ...
  const removeItem = (item: string) => {
    if (checked) return
    setPlaced(p => { const np = { ...p }; delete np[item]; return np })
  }
  const check = () => {
...
// ── Router component — no hooks, just picks the right component ───────────────
function QuestionRenderer({ game, question, onAnswer }: {
  ...
  if (type === 'true-false')    return <TrueFalseQ    question={question} onAnswer={onAnswer} />
  if (type === 'mcq')           return <McqQ          question={question} onAnswer={onAnswer} />
  if (type === 'flashcard')     return <FlashcardQ    question={question} onAnswer={onAnswer} />
  if (type === 'fill-blank')    return <FillBlankQ    question={question} onAnswer={onAnswer} />
  if (type === 'sort-sentence') return <SortSentenceQ question={question} onAnswer={onAnswer} />
  if (type === 'word-scramble') return <WordScrambleQ question={question} onAnswer={onAnswer} />
  if (type === 'match-words')   return <MatchWordsQ   question={question} onAnswer={onAnswer} />
  if (type === 'categorize')    return <CategorizeQ   question={question} onAnswer={onAnswer} />
